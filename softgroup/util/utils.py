...
def load_checkpoint(checkpoint, model, optimizer=None, strict=False):
    if hasattr(model, 'module'):
        model = model.module
    ...
    if missing_keys:
        tqdm.write(f'missing keys in source state_dict: {", ".join(missing_keys)}')
    if unexpected_keys:
        tqdm.write(f'unexpected key in source state_dict: {", ".join(unexpected_keys)}')
